...
<strong>简洁</strong><br>
    该作业旨在通过构建和训练一个对话代理系统来深入理解和实践机器学习算法的应用，特别是因子分解机（FM）、主动采样器及负采样器的预训练过程，并最终对所开发的对话代理进行评估。整个项目的实施分为四个主要步骤：首先是环境准备和知识图谱的构建，接着是对FM模型以及主动和负采样器的预训练，然后是对话代理的训练，最后是对对话代理性能的评估。所有训练模型和日志都将保存在特定的目录中，以便于后续分析和论文撰写。此项目不仅展示了团队成员在机器学习领域的技能与积累，同时也为未来的研究提供了有价值的参考和改进方向
以下是具体配置设置，请自行完成配置运行。
